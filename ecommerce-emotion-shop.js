    ...
    updateLoadingProgress(loaded, estimated, batch) {
        ...
        if (percentage >= 95) {
            ...
            if (allRecords.length === 100) {
                ...
                // Let's try a direct API test to see what's really happening
                try {
                    console.log('🔬 Testing direct API call...');
                    const testResponse = await axios.get(`https://api.airtable.com/v0/${baseId}/${tableName}?pageSize=1&fields[]=id`, {
                        headers: {
                            'Authorization': `Bearer ${apiKey}`,
                            'Content-Type': 'application/json'
                        },
